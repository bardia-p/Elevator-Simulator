...
public class UIClient extends ClientRPC implements Runnable{
	...
	private void processUpdate(Message m) {
		...
		if (m.getType() == MessageType.REQUEST) {
			...
			ui.doorOpened(openMessage.getID(), openMessage.getDirection(), openMessage.getArrivedFloor(), openMessage.getNumPickups(), openMessage.getNumDropoffs(), openMessage.getStopType(), openMessage.getTimestamp());
